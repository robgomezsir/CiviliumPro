...
import { atualizarLoteSchema } from "@civilium/shared";
import { and, eq, isNull } from "drizzle-orm";
import { db } from "@/db";
import { auditoriaEventos, lotes } from "@/db/schema";
import { actionClient } from "@/lib/safe-action";

export const atualizarLote = actionClient
  .schema(atualizarLoteSchema)
  .action(async ({ parsedInput }) => {
    await db.transaction(async (tx) => {
      ...
      const [depois] = await tx
        .update(lotes)
        .set({
          nomeArquivo: parsedInput.nomeArquivo.trim(),
          updatedAt: new Date(),
        })
